/**
 * whatsapp-bulk-meet-automation - NPM Library Entry
 * 
 * Usage as library:
 ...
 * 
 * Or functional:
 *   const wa = require('whatsapp-bulk-meet-automation');
...
class WhatsappBulk {
    ...
    constructor(opts = {}) {
        ...
        this.contactsCsv = opts.contactsCsv || this.config.paths.contactsCsv;
        this.templatePath = opts.templatePath || this.config.paths.messageTemplate;
        if (!opts.templatePath) {
            const detailed = path.join(__dirname, '..', 'data', 'message_template_detailed.txt');
            const fs = require('fs');
            if (fs.existsSync(detailed)) this.templatePath = './data/message_template_detailed.txt';
        }
...
module.exports = {
    ...
    // Helper
    sendFromCsv,
    // Default export is class for `require('whatsapp-bulk-meet-automation')` -> class
